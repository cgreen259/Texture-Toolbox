...
!
	INTEGER :: sizex, sizey, max_val, min_val
	INTEGER :: i_min, i_max, j_min, j_max
	INTEGER :: val1, val2, i2, j2
	REAL*8 :: glcm(0:max_val,0:max_val)
	REAL*8 :: s1, s2, value_add
			...
			do i2 = i_min, i_max
				do j2 = j_min, j_max
					if (i .NE. i2 .OR. j .NE. j2) then
						...
						if (val2 >= min_val) then
							!value_add = 1
							value_add = sqrt(abs(real(i2-i,8))+abs(real(j2-j,8)))
							glcm(val1, val2) = glcm(val1, val2) + value_add
						end if
					end if
				end do
			end do
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	SUBROUTINE glcm3d(image, glcm, max_val, min_val, &
		sizex, sizey, sizez)
	
!	Calculate GLCM of 3D array
	
	INTEGER :: sizex, sizey, sizez, max_val, min_val
							...
							do i2=i-1, i+1
								if (k2 .NE. k .OR. j2 .NE. j .OR. i2 .NE. i) then
									...
									if (val2 >= min_val) then
										idiff = abs(i2-i)
										! value_add = 1
										value_add = sqrt(real(idiff+jdiff+kdiff,8))
										glcm(val1, val2) = glcm(val1, val2) + value_add
									end if
								end if
							end do
